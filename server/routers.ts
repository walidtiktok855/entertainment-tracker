...
export const appRouter = router({
  ...
  tracker: router({
    ...
    promote: protectedProcedure.input(z.object({ id: z.number().int(), item: mediaInput.partial().extend({ type: z.enum(["Game", "Series", "Movie"]), status: z.string() }) })).mutation(async ({ ctx, input }) => {
      ...
      return updateMediaForUser(ctx.user.id, input.id, { ...input.item, scheduledDate: input.item.scheduledDate === undefined || input.item.scheduledDate === null ? input.item.scheduledDate : new Date(input.item.scheduledDate), favorite: input.item.favorite === undefined ? undefined : input.item.favorite ? 1 : 0, stage: "library", category: input.item.type === "Game" ? "Game" : input.item.type === "Movie" ? "Movie" : null, title: input.item.title || "Untitled", status: input.item.status });
    }),
    sync: protectedProcedure.input(z.object({ items: z.array(mediaInput) })).mutation(async ({ ctx, input }) => { for (const item of input.items) await upsertMediaForUser(ctx.user.id, { ...item, scheduledDate: item.scheduledDate == null ? null : new Date(item.scheduledDate), favorite: item.favorite ? 1 : 0 }); return getMediaForUser(ctx.user.id); }),
    update: protectedProcedure.input(z.object({ id: z.number().int(), patch: mediaInput.partial() })).mutation(({ ctx, input }) => updateMediaForUser(ctx.user.id, input.id, { ...input.patch, scheduledDate: input.patch.scheduledDate === undefined || input.patch.scheduledDate === null ? input.patch.scheduledDate : new Date(input.patch.scheduledDate), favorite: input.patch.favorite === undefined ? undefined : input.patch.favorite ? 1 : 0 })),
