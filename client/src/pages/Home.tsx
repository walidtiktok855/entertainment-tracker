...
function parseMetadata(value?: string) {
  ...
  try { return JSON.parse(value) as { source?: string; released?: string; releaseDate?: string; firstAirDate?: string; metacritic?: number; voteAverage?: number; overview?: string; originalLanguage?: string; tags?: string[]; stores?: string[]; platforms?: string[]; developers?: string[]; publishers?: string[] }; } catch { return null; }
}
...
function DetailModal({ item, onClose, onAdvance, onFavorite, isAuthenticated, onPlaytimeLogged }: { item: MediaItem; onClose: () => void; onAdvance: () => void; onFavorite: () => void; isAuthenticated: boolean; onPlaytimeLogged: (minutes: number) => void }) {
  const episodeCatalog = trpc.tracker.episodeCatalog.useQuery({ externalId: item.externalId ?? "" }, { enabled: item.type === "Series" && Boolean(item.externalId?.startsWith("tmdb:tv:")), retry: false });
  ...
  const episodeRows = episodeCatalog.data?.length ? episodeCatalog.data : Array.from({ length: 8 }, (_, index) => ({ seasonNumber: 2, episodeNumber: index + 1, title: `Episode ${index + 1}`, airDate: null, runtime: null, overview: "", image: "" }));
